...
import { BaseAddress, Credential, PrivateKey, Transaction, } from "@emurgo/cardano-serialization-lib-nodejs";
import { blake2b } from "blakejs";
import { FireblocksCardanoRawSDK } from "../FireblocksCardanoRawSDK.js";
import { ChainProviderCapability, Networks, } from "../types/index.js";
import { Logger, LogLevel } from "../utils/logger.js";
const addressFor = (paymentKey, stakeKey) => BaseAddress.new(0, Credential.from_keyhash(paymentKey.to_public().hash()), Credential.from_keyhash(stakeKey.to_public().hash()))
    .to_address()
    .to_bech32();
describe("Fireblocks governed ADA transfer", () => {
    beforeAll(() => Logger.setLogLevel(LogLevel.NONE));
    const createHarness = (authorizationStatus = "APPROVED", signerId = "designated-signer", transactionResponseId = "fireblocks-tx-123", fireblocksStatus = "COMPLETED", invalidSignature = false, authorizationLogic = "AND") => {
        ...
        const submitTransfer = jest.fn(async (cborHex) => {
        ...
        const signTransaction = jest.fn(async (payload) => {
            const content = payload.extraParameters.rawMessageData.messages[0].content;
            const signature = (invalidSignature ? PrivateKey.generate_ed25519() : paymentKey).sign(Buffer.from(content, "hex"));
            return {
    ...
    it("correlates preflight, Fireblocks policy evidence, signature, and Cardano hash", async () => {
        ...
        expect(harness.signTransaction).toHaveBeenCalledWith(expect.objectContaining({
            externalTxId: "cardano-governance-test-001",
            operation: "RAW",
        }));
    ...
    it("blocks a Demeter resource for the wrong Cardano network before signing", async () => {
        ...
        await expect(harness.sdk.transferAda({
            recipientAddress: harness.recipientAddress,
            lovelaceAmount: 2_000_000,
            governance: {
    ...
    it("rejects unknown Fireblocks authorization logic before submission", async () => {
        const harness = createHarness("APPROVED", "designated-signer", "fireblocks-tx-123", "COMPLETED", false, "UNKNOWN");
        await expect(harness.sdk.transferAda({
            recipientAddress: harness.recipientAddress,
            lovelaceAmount: 2_000_000,
            governance: {
    ...
    it("rejects mismatched Fireblocks response IDs before submission", async () => {
        const harness = createHarness("APPROVED", "designated-signer", "different-fireblocks-transaction");
        await expect(harness.sdk.transferAda({
            recipientAddress: harness.recipientAddress,
            lovelaceAmount: 2_000_000,
            governance: {
    ...
    it("requires Fireblocks COMPLETED status before submission", async () => {
        const harness = createHarness("APPROVED", "designated-signer", "fireblocks-tx-123", "BROADCASTING");
        await expect(harness.sdk.transferAda({
            recipientAddress: harness.recipientAddress,
            lovelaceAmount: 2_000_000,
            governance: {
    ...
    it("rejects a Fireblocks signature that does not verify against the body", async () => {
        const harness = createHarness("APPROVED", "designated-signer", "fireblocks-tx-123", "COMPLETED", true);
        await expect(harness.sdk.transferAda({
            recipientAddress: harness.recipientAddress,
            lovelaceAmount: 2_000_000,
            governance: {
...
//# sourceMappingURL=FireblocksCardanoRawSDK.governance.test.js.map