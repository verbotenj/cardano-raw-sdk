...
import { allowSignedWebhook, credentialsMatch, extractApiCredential, protectApi, validateServerApiKey, } from "../../api/security.js";
describe("HTTP server security", () => {
    ...
    it("reads x-api-key before a Bearer credential", () => {
        expect(extractApiCredential({
            headers: { "x-api-key": "header-key", authorization: "Bearer bearer-key" },
        })).toBe("header-key");
    });
    it("reads a case-insensitive Bearer credential", () => {
        expect(extractApiCredential({ headers: { authorization: "bearer test-key" } })).toBe("test-key");
    });
    ...
    it("allows only the exact signed webhook route to bypass application auth", () => {
        expect(allowSignedWebhook({ method: "POST", path: "/webhook" })).toBe(true);
        expect(allowSignedWebhook({ method: "GET", path: "/webhook" })).toBe(false);
        expect(allowSignedWebhook({ method: "POST", path: "/transfer/ada" })).toBe(false);
    });
