...
describe("Controller Helper Functions", () => {
    describe("getWebhookEnvironment", () => {
        ...
        it('should default to "US" for unknown values', () => {
            const result = mapBasePathToEnvironment("unknown");
            expect(result).toBe("US");
        });
        it('should handle undefined by defaulting to "US"', () => {
            const result = mapBasePathToEnvironment(undefined);
            expect(result).toBe("US");
        });
        it('should handle null by defaulting to "US"', () => {
            const result = mapBasePathToEnvironment(null);
            expect(result).toBe("US");
        });
    });
});
describe("Controller Error Handling", () => {
    describe("handleError with SdkApiError", () => {
        it("should format SdkApiError response correctly", () => {
            ...
            const error = new SdkApiError("Insufficient balance", 400, "InsufficientBalance", { required: 1000, available: 500 }, "FireblocksSDK");
        ...
        it("should include all SdkApiError fields in response", () => {
            const res = createMockResponse();
            const error = new SdkApiError("Validation failed", 400, "ValidationError", { field: "amount", reason: "negative" }, "CardanoSDK");
            res.status(400);
            res.json({
                success: false,
                error: error.message,
...
describe("Request Parameter Parsing", () => {
    describe("parseTransactionHistoryParams", () => {
        ...
        it("should parse index from query with default 0", () => {
            ...
            const index1 = req1.query?.index ? parseInt(req1.query.index, 10) : 0;
            expect(index1).toBe(0);
            const req2 = createMockRequest({
                query: { index: "5" },
            });
            const index2 = req2.query?.index ? parseInt(req2.query.index, 10) : 0;
...
function mapBasePathToEnvironment(basePath) {
    const path = basePath;
