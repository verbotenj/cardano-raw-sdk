...
describe("SdkApiError", () => {
    ...
    describe("With service name", () => {
        it("should store service name", () => {
            const error = new SdkApiError("API call failed", 500, "ServerError", undefined, "IagonApiService");
    ...
    describe("Complete error construction", () => {
        it("should create fully populated error", () => {
            ...
            const error = new SdkApiError("Amount below minimum UTXO value", 400, "BelowMinimumUtxo", errorInfo, "FireblocksCardanoRawSDK");
    ...
    describe("Real-world usage scenarios", () => {
        it("should create validation error for invalid input", () => {
            const error = new SdkApiError("lovelaceAmount must be a positive integer", 400, "ValidationError", { lovelaceAmount: -1000 }, "FireblocksCardanoRawSDK");
            expect(error.statusCode).toBe(400);
            expect(error.errorType).toBe("ValidationError");
        });
        it("should create insufficient balance error", () => {
            const error = new SdkApiError("Insufficient ADA balance", 400, "InsufficientBalance", { required: 5_000_000, available: 2_000_000 }, "FireblocksCardanoRawSDK");
            expect(error.errorType).toBe("InsufficientBalance");
            expect(error.errorInfo.required).toBe(5_000_000);
        });
        it("should create unsupported operation error", () => {
            const error = new SdkApiError("Native ADA transfers are not supported by this SDK", 400, "UnsupportedOperation", { tokenPolicyId: "", tokenName: "ADA" }, "FireblocksCardanoRawSDK");
            expect(error.errorType).toBe("UnsupportedOperation");
            expect(error.statusCode).toBe(400);
        });
        it("should create below minimum UTXO error", () => {
            const error = new SdkApiError("lovelaceAmount 500000 is below the Cardano protocol minimum of 1000000 lovelace", 400, "BelowMinimumUtxo", { lovelaceAmount: 500_000, minimum: 1_000_000 }, "FireblocksCardanoRawSDK");
            expect(error.errorType).toBe("BelowMinimumUtxo");
            expect(error.errorInfo.minimum).toBe(1_000_000);
        });
        it("should create server error without extra details", () => {
            const error = new SdkApiError("Unexpected error during transaction building", 500, "InternalError");
            expect(error.statusCode).toBe(500);
