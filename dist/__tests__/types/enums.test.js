...
import { Networks, TransactionType, SupportedAssets, GroupByOptions, WebhookEventTypes, RewardType, PoolStatus, DRepKind, CertificateType, DRepAction, StakingOperation, } from "../../types/enums.js";
describe("Networks", () => {
...
describe("WebhookEventTypes", () => {
    ...
    it("should have transaction approval status updated event", () => {
        expect(WebhookEventTypes.TRANSACTION_APPROVAL_STATUS_UPDATED).toBe("transaction.approval_status.updated");
    });
    it("should have network records processing completed event", () => {
        expect(WebhookEventTypes.TRANSACTION_NETWORK_RECORDS_PROCESSING_COMPLETED).toBe("transaction.network_records.processing_completed");
    });
...
describe("CertificateType", () => {
    ...
    it("should differentiate Shelley vs Conway registration types", () => {
        expect(CertificateType.STAKE_KEY_REGISTRATION).not.toBe(CertificateType.STAKE_REGISTRATION);
        expect(CertificateType.STAKE_KEY_DEREGISTRATION).not.toBe(CertificateType.STAKE_DEREGISTRATION);
    });
});
describe("DRepAction", () => {
