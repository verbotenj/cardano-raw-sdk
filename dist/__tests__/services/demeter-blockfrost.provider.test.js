...
import { ChainProviderCapability } from "../../types/index.js";
...
describe("DemeterBlockfrostProvider", () => {
    ...
    const provider = (options = {}) => new DemeterBlockfrostProvider({
        baseUrl: `${baseUrl}/`,
        apiKey: "demeter-test-key",
        ...options,
    });
    it("validates configuration and advertises only core capability", () => {
        expect(() => new DemeterBlockfrostProvider({ baseUrl: "", apiKey: "key" })).toThrow("DEMETER_BLOCKFROST_URL");
        expect(() => new DemeterBlockfrostProvider({ baseUrl, apiKey: "" })).toThrow("DEMETER_API_KEY");
        expect(() => new DemeterBlockfrostProvider({
    ...
    it("authenticates health, network, and latest-slot requests with dmtr-api-key", async () => {
        ...
        handler = (request, response) => {
            ...
            if (request.url === "/health")
                return json(response, 200, { is_healthy: true });
            if (request.url === "/genesis")
                return json(response, 200, { network_magic: 2 });
    ...
    it("normalizes flat and policy-grouped balances", async () => {
        ...
        await expect(provider().getBalanceByAddress({ address: "addr_test1example", groupByPolicy: false })).resolves.toEqual({
            success: true,
            data: { lovelace: 4200000, assets: { [`${policy}.4e4654`]: 7 } },
        });
        await expect(provider().getBalanceByAddress({ address: "addr_test1example", groupByPolicy: true })).resolves.toEqual({
            success: true,
            data: { lovelace: 4200000, assets: { [policy]: { "4e4654": 7 } } },
        });
    });
    it("paginates and normalizes multi-asset UTxOs", async () => {
        ...
        handler = (request, response) => {
            ...
            json(response, 200, Array.from({ length: count }, (_, index) => ({
    ...
    it("rejects malformed assets and unsafe numeric quantities", async () => {
        ...
        await expect(provider().getBalanceByAddress({ address: "addr_test1example", groupByPolicy: false })).rejects.toThrow("Unsafe numeric quantity");
        handler = (_request, response) => json(response, 200, {
            address: "addr_test1example",
            amount: [{ unit: "too-short", quantity: "1" }],
        });
        await expect(provider().getBalanceByAddress({ address: "addr_test1example", groupByPolicy: false })).rejects.toThrow("Invalid Blockfrost asset unit");
        handler = (_request, response) => json(response, 200, {
            address: "addr_test1example",
...
//# sourceMappingURL=demeter-blockfrost.provider.test.js.map