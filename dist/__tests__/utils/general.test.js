...
import { toHex, decodeAssetName, formatWithDecimals, parseAdaStringToLovelace, } from "../../utils/general.js";
describe("toHex", () => {
...
describe("formatWithDecimals", () => {
    ...
    describe("BigInt support", () => {
        it("should handle BigInt input for large values", () => {
            const result = formatWithDecimals(1000000n, 6);
            expect(result.value).toBe("1.000000");
            expect(result.raw).toBe("1,000,000");
        });
        it("should handle very large BigInt values", () => {
            const result = formatWithDecimals(45000000000000000n, 6);
            expect(result.value).toBe("45000000000.000000");
        });
    });
    ...
    describe("Edge cases and validation", () => {
        it("should throw error for negative amount", () => {
            expect(() => formatWithDecimals(-1000, 6)).toThrow("formatWithDecimals: negative amount is not supported");
        });
        it("should throw error for negative BigInt", () => {
            expect(() => formatWithDecimals(-1000n, 6)).toThrow("formatWithDecimals: negative amount is not supported");
        });
        it("should throw error for negative decimals", () => {
            expect(() => formatWithDecimals(1000, -1)).toThrow("formatWithDecimals: decimals must be a non-negative integer");
        });
        it("should throw error for non-integer decimals", () => {
            expect(() => formatWithDecimals(1000, 6.5)).toThrow("formatWithDecimals: decimals must be a non-negative integer");
        });
...
describe("parseAdaStringToLovelace", () => {
    ...
    describe("Edge cases", () => {
        ...
        it("should throw error for amounts exceeding safe integer range", () => {
            ...
            expect(() => parseAdaStringToLovelace("100000000000")).toThrow("ADA string out of range");
        });
