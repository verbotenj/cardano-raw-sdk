import { Fireblocks, TransactionOperation, TransferPeerPathType, } from "@fireblocks/ts-sdk";
...
export class FireblocksService {
    fireblocksSDK;
    logger = new Logger("services:fireblocks");
    /**
     ...
     */
    getVaultAccountAddress = async (vaultAccountId, assetId, index = 0) => {
        try {
            ...
            const filteredAddresses = addressesResponse.filter((addr) => addr.bip44AddressIndex === index);
            if (filteredAddresses.length === 0) {
                throw new Error(`No ${assetId} address found for vault account ${vaultAccountId} and index ${index}`);
            }
            return filteredAddresses[0];
        }
        catch (error) {
            const message = error instanceof Error ? error.message : "Unknown error";
            throw new Error(`Failed to get ${assetId} address for vault account ${vaultAccountId}: ${message}`, { cause: error });
        }
    };
    /**
     ...
     */
    getVaultAccountAddresses = async (vaultAccountId, assetId) => {
        try {
            const addressesResponse = await this.fireblocksSDK.vaults.getVaultAccountAssetAddressesPaginated({
                vaultAccountId,
                assetId,
            });
     ...
     */
    signTransaction = async (transactionPayload) => {
        try {
    ...
    createTransfer = async (params) => {
        ...
        const txId = transactionResponse.data.id;
        if (!txId)
            throw new Error("Transfer: transaction ID is undefined");
        const FIREBLOCKS_SUB_STATUS_MESSAGES = {
            AMOUNT_TOO_SMALL: "Insufficient asset amount remaining after transfer: the change output does not meet the minimum required",
        ...
        try {
            ...
            if (error instanceof Error) {
                ...
                if (subStatus) {
                    const message = FIREBLOCKS_SUB_STATUS_MESSAGES[subStatus] ?? `Transfer failed: ${subStatus}`;
        ...
        if (!txHash)
            throw new Error("Transfer completed but txHash is missing");
     ...
     */
    getAssetPublicKey = async (vaultAccountId, assetId, change = 0, addressIndex = 0) => {
        try {
            ...
            if (!publicKey) {
                throw new Error(`Error fetching public key for vault account ${vaultAccountId} on ${assetId}`);
            }
            return publicKey;
        }
        catch (error) {
            const message = error instanceof Error ? error.message : "Unknown error";
            throw new Error(`Failed to get public key for vault account ${vaultAccountId} on ${assetId}: ${message}`, { cause: error });
        }
    };
}
