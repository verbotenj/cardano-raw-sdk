...
import { CertificateType, DRepKind, Networks, DRepAction, } from "../types/index.js";
 ...
 */
export const blakeHash = (payload, digestSize = 28) => {
 ...
 */
export const decodeAddress = (encodedAddress, mainnet) => {
    const expectedHrp = getAddressHrp(mainnet);
    if (!encodedAddress.startsWith(`${expectedHrp}1`)) {
        throw new Error(`Address ${encodedAddress} is invalid (use Shelley-era addresses with ${expectedHrp} prefix)`);
    }
 ...
 */
export const getCertificateFromBaseAddress = (baseAddress, mainnet) => {
    const decoded = decodeAddress(baseAddress, mainnet);
    if (decoded.length < CardanoConstants.CARDANO_BASE_ADDRESS_MIN_LENGTH) {
        throw new Error(`Invalid base address length: ${decoded.length}, expected at least ${CardanoConstants.CARDANO_BASE_ADDRESS_MIN_LENGTH} bytes`);
    }
 ...
 */
export const getStakeAddressFromCertificate = (certificate, mainnet) => {
 ...
 */
export const getStakeAddressFromBaseAddress = (baseAddress, mainnet) => {
 ...
 */
export const buildRegistrationCertificate = (credential) => {
 ...
 * Shelley-era certs remain valid on the Conway ledger.
 */
export const buildDeregistrationCertificate = (credential) => {
 ...
 */
export const buildDelegationCertificate = (credential, poolId) => {
 ...
 */
export const buildVoteDelegationCertificate = (credential, drep) => {
 ...
 */
export const serializeWithdrawals = (withdrawals) => {
 ...
 */
export const embedSignaturesInTx = (deserializedTxPayload, signatures) => {
    ...
    const witnessesArr = sortedSignatures.map((sig) => [
...
export const buildPayload = (options) => {
    const { toAddress, txInputs, ttl, certificates, withdrawals, votingProcedures, network } = options;
    ...
    if (certificates && certificates.length > 0) {
        const certsArr = certificates.map((cert) => {
 ...
 */
export const calculateTtl = (currentSlot, ttlSecs = CardanoAmounts.TX_TTL_SECS) => {
    return currentSlot + ttlSecs;
};
export const findSuitableUtxo = (utxos, minAmount) => {
 ...
 */
const validateAnchorDataHash = (dataHash) => {
    if (!/^[0-9a-fA-F]{64}$/.test(dataHash)) {
        throw new Error(`Invalid anchor dataHash: must be exactly ${ANCHOR_DATA_HASH_HEX_LEN} hex characters (got "${dataHash.length}" chars)`);
    }
};
const decodeDRepId = (drepId) => {
    ...
    if (drepId.startsWith("drep1") || drepId.startsWith("drep_script1")) {
        ...
        if (fullBytes.length !== DREP_CREDENTIAL_BYTES + 1) {
            throw new Error(`Invalid DRep ID: bech32 payload must be ${DREP_CREDENTIAL_BYTES + 1} bytes, got ${fullBytes.length}`);
        }
    ...
    if (!/^[0-9a-fA-F]{56}$/.test(drepId)) {
        throw new Error(`Invalid DRep ID: hex format must be exactly ${DREP_CREDENTIAL_BYTES * 2} hex characters (${DREP_CREDENTIAL_BYTES} bytes), got length ${drepId.length}`);
    }
 ...
 */
export const drepActionToDRepInfo = (action, drepId) => {
    switch (action) {
 ...
 */
export const buildVotingProcedures = (credential, governanceActionId, vote, anchor) => {
    ...
    if (anchor)
        validateAnchorDataHash(anchor.dataHash);
 ...
 */
export const buildDRepRegistrationCertificate = (credential, depositLovelace, anchor) => {
 ...
 */
export const encodeDRepId = (credential, isScript = false) => {
...
//# sourceMappingURL=staking.utils.js.map