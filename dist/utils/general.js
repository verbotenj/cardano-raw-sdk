...
export const decodeAssetName = (assetId) => {
 ...
 */
export const formatWithDecimals = (rawAmount, decimals) => {
    if (!Number.isInteger(decimals) || decimals < 0) {
        throw new Error(`formatWithDecimals: decimals must be a non-negative integer (got ${decimals})`);
    }
 ...
 */
export const parseAdaStringToLovelace = (ada) => {
    if (!/^\d+(\.\d+)?$/.test(ada)) {
        throw new Error(`Invalid ADA string: "${ada}" - expected a non-negative decimal number (e.g. "1.5")`);
    }
    ...
    if (!Number.isSafeInteger(result)) {
        throw new Error(`ADA string out of range: "${ada}" - result ${result} lovelace exceeds safe integer bounds`);
    }
    return result;
};
