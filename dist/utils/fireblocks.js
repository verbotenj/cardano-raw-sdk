 ...
 * ```
 */
export const getTxStatus = async (txId, fireblocks, pollingInterval = 1000, maxWaitMs = 30 * 60 * 1000 // 30 minutes (Fireblocks RAW signing default timeout)
) => {
    try {
        let txResponse = await fireblocks.transactions.getTransaction({ txId });
        let lastStatus = txResponse.data.status;
        const deadline = Date.now() + maxWaitMs;
        logger.info(`Transaction ${txResponse.data.id} is currently at status - ${txResponse.data.status}`);
        // Poll until terminal state
        while (txResponse.data.status !== TransactionStateEnum.Completed &&
            txResponse.data.status !== TransactionStateEnum.Broadcasting) {
            if (Date.now() > deadline) {
                throw new Error(`Transaction ${txId} polling timed out after ${maxWaitMs / 1000}s. Last status: ${lastStatus}`);
            }
            ...
            if (txResponse.data.status !== lastStatus) {
                logger.info(`Transaction ${txResponse.data.id} status changed: ${lastStatus} → ${txResponse.data.status}`);
            ...
            switch (txResponse.data.status) {
                ...
                case TransactionStateEnum.Rejected:
                    throw new Error(`Transaction ${txResponse.data.id} failed with status: ${txResponse.data.status}\nSub-Status: ${txResponse.data.subStatus}`);
                default:
                    break;
            }
        }
        logger.info(`Transaction ${txResponse.data.id} reached terminal state: ${txResponse.data.status}`);
        return txResponse.data;
    }
    catch (error) {
        logger.error(`Error polling transaction ${txId}:`, error instanceof Error ? error.message : error);
        throw error;
    }
};
